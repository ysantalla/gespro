...
export class AuthService {
  ...
  private _isProfesor = new BehaviorSubject(false);
  private _isJProyecto = new BehaviorSubject(false);
  ...
  public isProfesor(): Observable<boolean> {
    return this._isProfesor.asObservable();
  }

  public isJProyecto(): Observable<boolean> {
    return this._isJProyecto.asObservable();
  }
  ...
  private setRoles(): void {
    ...
    if (this.getRoles().some(value => value.name === 'PROFESOR') && this.isLoggedIn()) {
      this._isProfesor.next(true);
    } else {
      this._isProfesor.next(false);
    }

    if (this.getRoles().some(value => value.name === 'JPROYECTO') && this.isLoggedIn()) {
      this._isJProyecto.next(true);
    } else {
      this._isJProyecto.next(false);
    }
  }
