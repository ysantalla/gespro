...
const deleteMes = gql`
  mutation deleteMes($where: MesWhereUniqueInput!) {
...
const deleteManyMess = gql`
  mutation deleteManyMeses($where: MesWhereInput!) {
            ...
            <div class="mat-elevation-z8">
              ...
              <mat-table #table [dataSource]="dataSource" matSort aria-label="Elements">
                ...
                <ng-container matColumnDef="nombre">
                  ...
                  <mat-cell fxFlex="49" *matCellDef="let row">{{row.nombre}}</mat-cell>
                </ng-container>
                ...
                <ng-container matColumnDef="details">
                  ...
                  <mat-cell fxFlex="7" *matCellDef="let row">
                    <a mat-icon-button color="accent" [routerLink]="['/admin','month', 'details', row.id]" routerLinkActive="active">
                ...
                <ng-container matColumnDef="edit">
                  ...
                  <mat-cell fxFlex="7" *matCellDef="let row">
                    <a mat-icon-button color="primary" [routerLink]="['/admin','month', 'update', row.id]" routerLinkActive="active">
...
export class MesListComponent implements OnInit, AfterViewInit, OnDestroy {
  dataSource: MatTableDataSource<Meses.Meses>;

  selection = new SelectionModel<Meses.Meses>(true, []);

  /** Columns displayed in the table. Columns IDs can be added, removed, or reordered. */
  displayedColumns = ['select', 'nombre', 'habilitado', 'details', 'edit', 'delete'];
    ...
    this.querySubscription = this.apollo
      ...
      .valueChanges.subscribe(
        ({ data, loading }) => {
          ...
          if (data) {
            this.dataSource.data = data.meses;
          }
        },
        error => {
  ...
  onDelete(mes: Meses.Meses): void {
    const dialogRef = this.dialog.open(ConfirmComponent, {
      data: {
        message: `¿Está seguro que desea eliminar el mes "${mes.nombre}"?`,
      }
    });
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.apollo.mutate({
          ...
          if (data) {
            this.dataSource.data = this.dataSource.data.filter(meses => meses.id !== mes.id);
            this.snackBar.open(`Mes ${data.deleteMes.nombre} eliminado correctamente`, 'X', {duration: 3000});
          }
        }, (error) => {
          this.loading = false;
  ...
  onDeleteSelected($event): void {
    const dialogRef = this.dialog.open(ConfirmComponent, {
      data: {
        message: `¿Está seguro que desea eliminar los ${this.selection.selected.length} meses selecionados?`,
      }
    });
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.apollo.mutate({
          ...
          if (data) {
            this.dataSource.data = this.dataSource.data.filter(mes => !this._mesExist(mes, this.selection.selected));
            this.selection.clear();
            this.snackBar.open(`Los ${data.deleteManyMeses.count} meses fueron eliminados correctamente`, 'X', {duration: 3000});
          }
        }, (error) => {
          this.loading = false;
          if (error.graphQLErrors.length > 0) {
  ...
  private _mesExist(mes: Meses.Meses, meses: any): boolean {
    if (meses.some(data => (data.id === mes.id))) {
      return true;
    }
    return false;
  }
