...
const deleteLinea = gql`
  mutation deleteLinea($where: LineaWhereUniqueInput!) {
...
const deleteManyLineas = gql`
  mutation deleteManyLineas($where: LineaWhereInput!) {
    ...
    <div class="container">
      ...
      <div fxLayout="row" fxLayoutAlign="center center">
        <div class="item" fxFlex="98%">
          <button class="create-button" mat-raised-button color="primary" routerLink="/admin/linea/create" routerLinkActive="active">
              ...
              <mat-table #table [dataSource]="dataSource" matSort aria-label="Elements">
                ...
                <ng-container matColumnDef="details">
                  ...
                  <mat-cell fxFlex="7" *matCellDef="let row">
                    <a mat-icon-button color="accent" [routerLink]="['/admin','linea', 'details', row.id]" routerLinkActive="active">
                ...
                <ng-container matColumnDef="edit">
                  ...
                  <mat-cell fxFlex="7" *matCellDef="let row">
                    <a mat-icon-button color="primary" [routerLink]="['/admin','linea', 'update', row.id]" routerLinkActive="active">
...
export class LineaListComponent implements OnInit, AfterViewInit, OnDestroy {
  dataSource: MatTableDataSource<Linea.Linea>;

  selection = new SelectionModel<Linea.Linea>(true, []);
    ...
    this.querySubscription = this.apollo
      ...
      .valueChanges.subscribe(
        ({ data, loading }) => {
          ...
          if (data) {
            this.dataSource.data = data.lineas;
          }
        },
        error => {
  ...
  onDelete(linea: Linea.Linea): void {
    const dialogRef = this.dialog.open(ConfirmComponent, {
      data: {
        message: `¿Está seguro que desea eliminar el linea "${linea.nombre}"?`,
      }
    });
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.apollo.mutate({
          ...
          if (data) {
            this.dataSource.data = this.dataSource.data.filter(lineas => lineas.id !== linea.id);
            this.snackBar.open(`Tipo ${data.deleteLinea.nombre} eliminado correctamente`, 'X', {duration: 3000});
          }
        }, (error) => {
          this.loading = false;
  ...
  onDeleteSelected($event): void {
    const dialogRef = this.dialog.open(ConfirmComponent, {
      data: {
        message: `¿Está seguro que desea eliminar los ${this.selection.selected.length} lineas selecionados?`,
      }
    });
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.apollo.mutate({
          ...
          if (data) {
            this.dataSource.data = this.dataSource.data.filter(linea => !this._lineaExist(linea, this.selection.selected));
            this.selection.clear();
            this.snackBar.open(`Los ${data.deleteManyLineas.count} lineas fueron eliminados correctamente`, 'X', {duration: 3000});
          }
        }, (error) => {
          this.loading = false;
          if (error.graphQLErrors.length > 0) {
  ...
  private _lineaExist(linea: Linea.Linea, lineas: any): boolean {
    if (lineas.some(data => (data.id === linea.id))) {
      return true;
    }
    return false;
  }
