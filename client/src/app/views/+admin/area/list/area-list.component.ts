...
@Component({
  ...
  template: `
    <div class="container">
      ...
      <div fxLayout="row" fxLayoutAlign="center center">
        <div class="item" fxFlex="98%">
          <button class="create-button" mat-raised-button color="primary" routerLink="/admin/area/create" routerLinkActive="active">
              ...
              <mat-table #table [dataSource]="dataSource" matSort aria-label="Elements">
                ...
                <ng-container matColumnDef="details">
                  ...
                  <mat-cell fxFlex="7" *matCellDef="let row">
                    <a mat-icon-button color="accent" [routerLink]="['/admin','area', 'details', row.id]" routerLinkActive="active">
                ...
                <ng-container matColumnDef="edit">
                  ...
                  <mat-cell fxFlex="7" *matCellDef="let row">
                    <a mat-icon-button color="primary" [routerLink]="['/admin','area', 'update', row.id]" routerLinkActive="active">
...
export class AreaListComponent implements OnInit, AfterViewInit, OnDestroy {
  dataSource: MatTableDataSource<Area.Area>;

  selection = new SelectionModel<Area.Area>(true, []);
    ...
    this.querySubscription = this.apollo
      ...
      .valueChanges.subscribe(
        ({ data, loading }) => {
          ...
          if (data) {
            this.dataSource.data = data.areas;
          }
        },
        error => {
  ...
  onDelete(area: Area.Area): void {
    const dialogRef = this.dialog.open(ConfirmComponent, {
      data: {
        message: `¿Está seguro que desea eliminar el area "${area.nombre}"?`,
      }
    });
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.apollo.mutate({
          ...
          if (data) {
            this.dataSource.data = this.dataSource.data.filter(areas => areas.id !== area.id);
            this.snackBar.open(`Tipo ${data.deleteArea.nombre} eliminado correctamente`, 'X', {duration: 3000});
          }
        }, (error) => {
          this.loading = false;
  ...
  onDeleteSelected($event): void {
    const dialogRef = this.dialog.open(ConfirmComponent, {
      data: {
        message: `¿Está seguro que desea eliminar los ${this.selection.selected.length} areas selecionados?`,
      }
    });
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.apollo.mutate({
          ...
          if (data) {
            this.dataSource.data = this.dataSource.data.filter(area => !this._areaExist(area, this.selection.selected));
            this.selection.clear();
            this.snackBar.open(`Los ${data.deleteManyAreas.count} areas fueron eliminados correctamente`, 'X', {duration: 3000});
          }
        }, (error) => {
          this.loading = false;
          if (error.graphQLErrors.length > 0) {
  ...
  private _areaExist(area: Area.Area, areas: any): boolean {
    if (areas.some(data => (data.id === area.id))) {
      return true;
    }
    return false;
  }
